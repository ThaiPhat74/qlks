...
def RoomType_stats(kw=None,from_date=None,to_date=None):
    # rt = db.session.query(Room.id,Room.name,func.sum(Receipt.totalPrice * func.count()))\
    # rt = db.session.query(Room.id,RoomType.id,RoomType.name, func.count(Booking.id).label('so_lan_xuat_hien'),
    #                            func.sum(Receipt.totalPrice).label('tong_gia')) \
    #     .outerjoin(RoomType, RoomType.id == Room.roomType_id) \
    #     .outerjoin(Booking, Booking.room_id == Room.id) \
    #     .outerjoin(Receipt, Receipt.booking_id == Booking.id) \
    #     .group_by(Room.id,RoomType.id, RoomType.name)
    # if kw:
    #     rt = rt.filter(Room.name.contains(kw))
    # if from_date:
    #     rt = rt.filter(Receipt.startDay.__ge__(from_date))
    # if to_date:
    #     rt = rt.filter(Receipt.startDay.__le__(to_date))
    #
    # return rt.all()
    rt = db.session.query(RoomType.id, RoomType.name, func.count(Booking.id).label('so_lan_xuat_hien'),
                          func.sum(Receipt.totalPrice).label('tong_gia')) \
        .outerjoin(Room, Room.roomType_id == RoomType.id) \
        .outerjoin(Booking, Booking.room_id == Room.id) \
        .outerjoin(Receipt, Receipt.booking_id == Booking.id) \
        .group_by(RoomType.id, RoomType.name)
    if kw:
        rt = rt.filter(RoomType.name.contains(kw))
    if from_date:
        rt = rt.filter(Receipt.startDay.__ge__(from_date))
    if to_date:
        rt = rt.filter(Receipt.startDay.__le__(to_date))
...
def roomType_month_stats(year):
    query = db.session.query(
        extract('month', Receipt.startDay).label('month'),
        func.sum(Receipt.totalPrice).label('tong_gia')
            )

    query = query.filter(extract('year', Receipt.startDay) == year)
    query = query.group_by(extract('month', Receipt.startDay))
